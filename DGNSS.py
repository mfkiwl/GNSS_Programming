...
def SD_onPseudorange(knownStation_ob_records, unknownStation_ob_records, br_records,
    ...
    """
    # 筛选出某时刻的记录
    the_knownStation_ob_record = list(filter(lambda o:o.SVN[0]=="G" and o.time==Tr and o.data!="",knownStation_ob_records))
    the_unknownStation_ob_record = list(filter(lambda o:o.SVN[0]=="G" and o.time==Tr and o.data!="",unknownStation_ob_records))
    ...
    while True:
        ...
        for unknownStation_record in the_unknownStation_ob_record:
            ...
            knownStation_record_base = list(filter(lambda o: o.SVN == unknownStation_record.SVN, the_knownStation_ob_record))
            ...
            # the_prn = unknownStation_record.SVN
            ...
            #     # ts = Ts.minus(dts2)
            # # 修正地球自转影响
            # dt = TimeSystem.cal_GPSws_deltatime_inseconds(tr, ts)
            ...
            P_u = unknownStation_record.data['P1']['observation']
            P_k = knownStation_record.data['P1']['observation']
            the_svn = unknownStation_record.SVN
            ts, dts = SPP.cal_EmitTime_from_datetime(Tr, the_svn, P_u, br_records, doCRC=True)
        ...
        if abs(dXu) < 1e-4 and abs(dYu) < 1e-4 and abs(dZu) < 1e-4:
            ...
            v = A @ x - l
            r = len(Ps) - len(x)
            sigma = math.sqrt((v.T @ Pz @ v) / r)
            break
    return Xu, Yu, Zu, sigma
...
if __name__ == "__main__":
    ...
    # unknownStation_observation_file = r"edata\obs\ptbb3100.20o"
    knownStation_observation_file = r"edata\obs\leij3100.20o"
    broadcast_file = r"edata\sat_obit\brdc3100.20n"
    # 读入观测文件内容,得到类型对象列表
    unknownStation_ob_records = DoFile.read_Rinex2_oFile(unknownStation_observation_file)
    knownStation_ob_records = DoFile.read_Rinex2_oFile(knownStation_observation_file)
    ...
    while Tr < datetime.datetime(2020, 11, 5, 23, 45, 16):
        Xk, Yk, Zk, sig = SD_onPseudorange(knownStation_ob_records, unknownStation_ob_records, br_records, Tr, knownStation_coor)
        print(sig)
        sigs.append(sig)
